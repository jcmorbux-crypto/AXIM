"""AXIM Trader V2 - Money Management Studio, the 4-strategy redesign.
...
of having a professional trading coach beside you.

Exactly 4 official, LOCKED strategies + 1 Custom Strategy Builder entry
point. No performance numbers (ROI/drawdown/win-rate) live here - those
depend entirely on a specific Signal Provider's real historical signals
and belong only in Strategy Lab / Provider Analysis. Every number below
is a mechanical, deterministic illustration of the RULES (what happens
on the very next trade, and at each growth checkpoint) - never a
performance projection, never fabricated.

Not yet wired to core/risk_engine.py: the growth-recalculation-on-
threshold mechanic (baseline resets at a % growth checkpoint) has no
equivalent in the real backend's compounding model today (which only
knows profit-threshold/daily/weekly/milestone triggers, not "trailing
baseline reset at +125%"). Building that is real backend work, out of
scope for a UI-Vision-branch product redesign - tracked as a follow-up,
not silently skipped. Every worked-example/timeline number below is
computed by this module's own pure functions, not core/risk_engine.py.
"""
